...
async function fetchLessonUrls(courseUrl: string): Promise<PageTitleAndLink[]> {
  ...
  const pageLinks = await page.evaluate(() => {
    ...
    if (document.location.pathname.includes('/module/')) {
      links = Array.from(document.querySelectorAll('.items-start .w-full.mx-0.mt-0.flex.flex-col a'));
    } else {
      document.querySelectorAll('.course-category-hover .feather.feather-chevron-down').forEach((item) => {
        const category = item.parentNode as HTMLElement;
        return category.click();
      })
      links = Array.from(document.querySelectorAll('#lesson-wrapper a'));
    }
  ...
  // marking list folders
  const listHeadings = await page.evaluate(() => {
    let headings: string;
    if (document.location.pathname.includes('/module/')) {
    ...
    await page.evaluate(() => {
      ...
      (<any>window).showSolution = async function showSolution() {
        ...
        try {
          ...
          const showSolutionXPath = document.evaluate('//button[text()="Got it!"]', document, null, XPathResult.ORDERED_NODE_SNAPSHOT_TYPE, null);
